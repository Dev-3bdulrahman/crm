<div class="p-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
            ...
            <p class="text-gray-500 text-sm mt-1">{{ __('crm::crm.customer_details') }}</p>
        </div>
        <div>
            <a href="{{ route('admin.crm.customers.edit', $customer->id) }}"
                ...
                <span>{{ __('crm::crm.edit_customer') }}</span>
            </a>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <span class="text-sm text-gray-900 dark:text-white" dir="ltr">{{ $customer->phone ?: '-' }}</span>
            </div>
            <div>
                <span class="block text-xs font-bold text-gray-400 uppercase mb-1">{{ __('crm::crm.address') }}</span>
                <span class="text-sm text-gray-900 dark:text-white">{{ $customer->address ?: '-' }}</span>
            </div>
            <div>
                <span class="block text-xs font-bold text-gray-400 uppercase mb-1">{{ __('crm::crm.customer_group') }}</span>
                <span class="text-sm text-gray-900 dark:text-white">{{ $customer->group?->name ?: '-' }}</span>
            </div>
            <div>
                <span class="block text-xs font-bold text-gray-400 uppercase mb-1">{{ __('crm::crm.organization') }}</span>
                <span class="text-sm text-gray-900 dark:text-white">{{ $customer->organization?->name ?: '-' }}</span>
            </div>
            <div>
                <span class="block text-xs font-bold text-gray-400 uppercase mb-1">{{ __('crm::crm.contact') }}</span>
                <span class="text-sm text-gray-900 dark:text-white">{{ $customer->contact?->name ?: '-' }}</span>
            </div>
            <div>
                <span class="block text-xs font-bold text-gray-400 uppercase mb-1">{{ __('crm::crm.status') }}</span>
                <span class="px-2.5 py-1 rounded-full text-xs font-semibold {{ $customer->status === 'active' ? 'bg-green-50 text-green-700 dark:bg-green-950/30 dark:text-green-400' : 'bg-gray-50 text-gray-650 dark:bg-gray-800 dark:text-gray-400' }}">
                    {{ __('crm::crm.status_' . $customer->status) }}
                </span>
            </div>
            <div>
                <span class="block text-xs font-bold text-gray-400 uppercase mb-1">{{ __('crm::crm.created_at') }}</span>
                <span class="text-sm text-gray-900 dark:text-white">{{ $customer->created_at->format('Y-m-d') }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm p-6 mb-6">
        <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">{{ __('crm::crm.opportunities') }}</h3>
        @if($customer->opportunities && $customer->opportunities->count())
            <div class="overflow-x-auto">
                <table class="w-full text-right border-collapse">
                    <thead>
                        <tr class="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
                            <th class="px-4 py-3 text-xs font-bold text-gray-400 uppercase">{{ __('crm::crm.name') }}</th>
                            <th class="px-4 py-3 text-xs font-bold text-gray-400 uppercase">{{ __('crm::crm.value') }}</th>
                            <th class="px-4 py-3 text-xs font-bold text-gray-400 uppercase">{{ __('crm::crm.status') }}</th>
                            <th class="px-4 py-3 text-xs font-bold text-gray-400 uppercase">{{ __('crm::crm.pipeline_stage') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                        @foreach($customer->opportunities as $opportunity)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                                <td class="px-4 py-3 font-semibold text-gray-900 dark:text-white text-sm">{{ $opportunity->name }}</td>
                                <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{{ number_format($opportunity->value, 2) }}</td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{{ $opportunity->pipelineStage?->name ?: '-' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        @else
            <div class="text-center py-8 text-gray-500">
                <i data-lucide="briefcase" class="w-8 h-8 mx-auto mb-2 text-gray-300"></i>
                <span>{{ __('crm::crm.no_opportunities') }}</span>
            </div>
        @endif
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm p-6 mb-6">
        ...
        <!-- Activities List -->
        @if($customer->activities && $customer->activities->count())
            <div class="space-y-3">
                {{-- Pending Activities --}}
                @foreach($customer->activities->where('status', 'pending') as $activity)
                ...
                @foreach($customer->activities->where('status', 'completed') as $activity)
